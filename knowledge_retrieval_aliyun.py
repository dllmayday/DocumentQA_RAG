...
"""
-------------------------------------------------
   @File Name:     knowledge_retrieval_aliyun.py
   @Author:        Luyao.zhang
   @Date:          2023/12/29
   @Description:   RAG retrieval with Aliyun Bailian API
-------------------------------------------------
"""
import os
...
# ------------ Custom Embeddings for Aliyun Bailian -----------
class AliyunEmbeddings(Embeddings):
    """Custom embeddings class for Aliyun Bailian API"""
    def __init__(self, api_key: str, base_url: str, model: str = "text-embedding-v3"):
        self.client = OpenAI(api_key=api_key, base_url=base_url)
        self.model = model
        self.batch_size = 10  # Aliyun API limit
    
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """Embed a list of documents (max 10 per batch)"""
        all_embeddings = []
...
def get_embeddings_model(model: str = "text-embedding-v3"):
    """获取 Aliyun 嵌入模型"""
    return AliyunEmbeddings(
        api_key=openai_api_key,
        base_url=emb_openai_api_base,
        model=model
    )


def get_chat_model(model: str = "qwen-plus"):
    """获取 Aliyun 聊天模型"""
    return ChatOpenAI(
        api_key=openai_api_key,
        base_url=chat_openai_api_base,
        model=model,
        temperature=0.1
    )
...
def main():
    while True:
        ...
        # 使用 Aliyun API
        embeddings = get_embeddings_model(model="text-embedding-v3")
        rag = RAG_pipeline(
